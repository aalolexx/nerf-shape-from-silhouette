...
class TSDF:
    ...
    def get_mesh(self) -> Mesh:
        """Extracts a mesh using marching cubes and filters out small components."""
        ...
        initial_mesh = Mesh(vertices=vertices, faces=faces, normals=normals, colors=colors)

        # Filter out small components using PyMeshLab
        # final_mesh = self.filter_small_components(initial_mesh, min_size=50)  # Set the min_size threshold as needed

        return initial_mesh

    def filter_small_components(self, mesh: Mesh, min_size: int) -> Mesh:
        """Filters out small connected components from the mesh."""
        
        # Convert the mesh to PyMeshLab format
        vertex_matrix = mesh.vertices.cpu().numpy().astype("float64")
        face_matrix = mesh.faces.cpu().numpy().astype("int32")
        color_matrix = mesh.colors.cpu().numpy().astype("float64")
        normal_matrix = mesh.normals.cpu().numpy().astype("float64")

        # Add an alpha channel to the color matrix
        alpha_channel = np.ones((color_matrix.shape[0], 1))
        color_matrix_with_alpha = np.concatenate([color_matrix, alpha_channel], axis=1)

        pm_mesh = pymeshlab.Mesh(
            vertex_matrix=vertex_matrix, 
            face_matrix=face_matrix, 
            v_color_matrix=color_matrix_with_alpha, 
            v_normals_matrix=normal_matrix
        )
        ms = pymeshlab.MeshSet()
        ms.add_mesh(pm_mesh, "mesh")
        
        # Apply the filter to remove small connected components
        ms.apply_filter('meshing_remove_connected_component_by_faces', mincomponentsize=min_size)

        # Get the filtered mesh
        filtered_mesh = ms.current_mesh()

        # Convert back to the Mesh class format
        vertices = torch.from_numpy(filtered_mesh.vertex_matrix()).to(mesh.vertices.device)
        faces = torch.from_numpy(filtered_mesh.face_matrix()).to(mesh.faces.device)
        colors = torch.from_numpy(filtered_mesh.vertex_color_matrix()).to(mesh.colors.device)
        normals = torch.from_numpy(filtered_mesh.vertex_normal_matrix()).to(mesh.normals.device)
    ...
    def integrate_tsdf(
        ...
        for i in range(batch_size):
            ...
            #     print(f"old_colors_i shape: {old_colors_i.shape}")
            #     print(f"new_colors_i shape: {new_colors_i.shape}")
            #     print(f"valid_points_i_shape: {valid_points_i_shape.shape}")
            #     self.colors[valid_points_i_shape] = (
            #         old_colors_i * old_weights_i[:, None] + new_colors_i * new_weights_i
            #     ) / total_weights[:, None]

            ## adapted for alex' model (ading one dim to new_colors)
            # TODO: old colours ideally should be M,2 too
            if sampled_colors is not None:
                old_colors_i = self.colors[valid_points_i_shape]  # [M, 3]
                new_colors_i = sampled_colors[i][:, valid_points_i.squeeze(0)].permute(1, 0)  # [M, 2]

                # Ensure new_colors_i has 3 channels by adding a third channel
                if new_colors_i.shape[1] == 2:
                    new_colors_i = torch.cat([new_colors_i, torch.ones_like(new_colors_i[:, :1])], dim=1)
