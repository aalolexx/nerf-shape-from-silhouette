...
class BinaryField(Field):
    ...
    # TODO: simplify output to binary
    ...
    # TODO: I think all this can kidn of be removed
    def get_outputs(
        ...
        # Apply the MLP and reshape output to get RGB values
        # rgb = self.mlp_head(h).view(*outputs_shape, -1).to(directions)
        # Compute the shape based on the outputs_shape
        rgb_shape = (*outputs_shape, 3)

        # Dummy rgb value for circumventing the training computation
        rgb = torch.full(rgb_shape, 0.5, device=directions.device, dtype=directions.dtype)

        # # Print detailed information about the rgb tensor
        # print("-----RGB Tensor Information-----")
        # print(f"Shape: {rgb.shape}, Data Type: {rgb.dtype}, Device: {rgb.device}")
        # print(f"Min value: {rgb.min().item()}, Max value: {rgb.max().item()}")

        # # Print the first few elements of the tensor to inspect the data
        # print("First few elements of rgb:")
        # print(rgb[:5])  # Adjust the slicing as needed to get a reasonable view

        # # Ensure there are no NaN values
        # if torch.isnan(rgb).any():
        #     print("Warning: rgb contains NaN values")
        # else:
        #     print("No NaN values in rgb")

        # Update outputs with RGB values
